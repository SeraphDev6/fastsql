...
class DBTable:
    ...
    @property
    def pks(self) -> tuple:
        return tuple(self.table.primary_key) + tuple(
            self.table.c[o] for o in self.xtra_id.keys()
        )
...
def _column(name, typ, primary=False):
    return sa.Column(name, _type_map.get(typ, typ), primary_key=primary)
...
def insert(self: DBTable, obj):
    ...
    result = self.conn.execute(
        sa.insert(self.table).values(**d).returning(*self.table.columns)
    )
...
def __call__(
    ...
    where_args: (
        Iterable | dict | NoneType
    ) = None,  # Parameters to use with `where`; iterable for `id>?`, or dict for `id>:id`
    ...
    **kw,  # Combined with `where_args`
    ...
    if where:
        where = sa.text(where)
        if kw:
            where = where.bindparams(**kw)
        query = query.where(where)
    if order_by:
        query = query.order_by(sa.text(order_by))
...
def __contains__(
    ...
    if isinstance(pk_values, (str, int)):
        pk_values = (pk_values,)
    try:
        self[pk_values]
        return True
